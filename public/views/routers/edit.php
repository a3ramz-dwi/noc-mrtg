 ...
 * Router Edit View
 *
 * @var array  $router  Existing router record
 * @var string $csrf    CSRF token
 */
$pageTitle = 'Edit Router: ' . htmlspecialchars($router['name'] ?? '');
?>
<div id="router-edit-page">
  ...
  <div class="mb-4">
    ...
    <h2 class="page-title mt-1">
      ...
      Edit: <?= htmlspecialchars($router['name'] ?? '') ?>
    </h2>
  </div>
  ...
  <div class="card" style="max-width:640px;">
    <div class="card-body">
      ...
      <form method="POST" action="/routers/<?= (int) $router['id'] ?>" id="router-form">
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="ip_address" name="ip_address"
                 value="<?= htmlspecialchars($router['ip_address'] ?? '') ?>" required>
        </div>
          ...
          <div class="col-md-3 mb-3">
            ...
            <select class="form-select" id="snmp_version" name="snmp_version">
              <?php foreach (['1', '2c'] as $v): ?>
                <option value="<?= $v ?>" <?= ($router['snmp_version'] ?? '2c') === $v ? 'selected' : '' ?>>
                  v<?= $v ?>
                </option>
              <?php endforeach; ?>
            </select>
