 ...
 *
 * @var array $user  PPPoE user record with traffic stats
 */
$pageTitle = 'PPPoE: ' . htmlspecialchars($user['username'] ?? '');
?>
<div id="pppoe-show-page">
  ...
  <div class="mb-4">
    ...
    <h2 class="page-title mt-1">
      ...
      <?= htmlspecialchars($user['username'] ?? 'PPPoE User') ?>
    </h2>
  </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <dl class="row mb-0">
            ...
            <dd class="col-sm-7">
              <a href="/routers/<?= (int) $user['router_id'] ?>">
                <?= htmlspecialchars($user['router_name'] ?? '—') ?>
              </a>
            </dd>
            ...
            <dd class="col-sm-7"><code><?= htmlspecialchars($user['ip_address'] ?? '—') ?></code></dd>

            <dt class="col-sm-5 text-muted">MAC Address</dt>
            <dd class="col-sm-7"><code><?= htmlspecialchars($user['mac_address'] ?? '—') ?></code></dd>

            <dt class="col-sm-5 text-muted">Service</dt>
            <dd class="col-sm-7"><?= htmlspecialchars($user['service'] ?? '—') ?></dd>

            <dt class="col-sm-5 text-muted">Profile</dt>
            <dd class="col-sm-7"><?= htmlspecialchars($user['profile'] ?? '—') ?></dd>

            <dt class="col-sm-5 text-muted">Status</dt>
            <dd class="col-sm-7">
              <?php $status = $user['status'] ?? 'disconnected'; ?>
            ...
            <dd class="col-sm-7"><?= htmlspecialchars($user['uptime'] ?? '—') ?></dd>
